...
- ## Открытие и сохранение файла
  ...
  ![image](https://github.com/user-attachments/assets/c6905b20-fd9e-432a-b391-5dfa9fd46b4d)
...
- ## Настройки
  ...
  - ### Общие настройки
    ...
    ![image](https://github.com/user-attachments/assets/b659afcb-2f2c-412c-a9bc-04e1e337f3bb)
    ...
    - #### Создание музыки с помощью ИИ
      ...
      **Кнопки обновления/сброса:** Управление жизненным циклом ключа API. Кнопка обновления отключена, вероятно, ожидая корректного ввода данных.
      ...
      - ##### **Условия использования API Google Gemini™ и проверка возраста**
      
      При первом вводе ключа API появляется диалоговое окно с Условиями использования Google Gemini™. Чтобы соответствовать требованиям API и возрастным ограничениям, пользователям необходимо:
      
        - **Ознакомиться и принять Условия использования** перед использованием функций API Google Gemini™.
        - **Ввести дату рождения**, чтобы подтвердить, что им исполнилось 18 лет. Лицам младше 18 лет запрещено использовать функции API Google Gemini™ в NeoBleeper.
        - Если предоставленный ключ API недействителен, пользователю меньше 18 лет или условия использования не приняты, ключ будет отклонен, и функции API Google Gemini™ не будут включены.
        - **Ваша дата рождения не будет храниться или передаваться** NeoBleeper; она используется однократно только для проверки возраста и соответствия требованиям.
    
        ![image1](https://github.com/user-attachments/assets/ffcbe66a-a145-4c89-9d89-2df67192ffa0)
            
        Соблюдение возрастного ценза и принятие условий использования являются обязательными при первом вводе нового ключа API. Это гарантирует ответственное использование и соблюдение юридических норм при использовании музыкальных функций, генерируемых ИИ.
  
    - #### Тестирование системного динамика

    NeoBleeper предоставляет надежные инструменты для проверки выходного сигнала системного динамика и диагностики проблем с подключением и совместимостью. В разделе «Тестирование системного динамика» на вкладке «Общие» настроек вы найдете две отдельные функции тестирования:

    ![image](https://github.com/user-attachments/assets/3428016b-69aa-42ed-b22b-72b5440883ea)
    
    **Тест системного динамика:**
    - Нажмите эту кнопку, чтобы открыть раскрывающийся список с двумя вариантами:
    - **Тест воспроизведения стандартной мелодии:** Воспроизводит простую мелодию, чтобы проверить, воспроизводит ли системный динамик ожидаемый звук.
    - **Расширенный тест системного динамика:** Запускает серию диагностических проверок для тщательной оценки функциональности динамика и стабильности работы драйвера (подробности см. ниже).
    
    **Расширенные функции проверки системных динамиков**
    При выборе пункта «Расширенная проверка системных динамиков» из раскрывающегося списка NeoBleeper выполняет несколько проверок конфигурации вашего оборудования и программного обеспечения, отображая результаты во всплывающем окне:

    ![image1](https://github.com/user-attachments/assets/d8b863ef-aa5f-47a3-84ea-2fb4ef39721d)
    
    Выполняются следующие тесты:
    - **Тест электрической обратной связи:** Проверяет ожидаемый электрический отклик системных динамиков.
    - **Тест стабильности состояния портов:** Проверяет стабильное состояние портов и стабильность драйвера.
    - **Расширенный тест частотной развертки:** Запускает различные частоты для обеспечения широкого диапазона тонов.
    - **Общий результат:** Сводка, показывающая, пройдены ли все тесты или обнаружена конкретная проблема.
    
    Каждая проверка отображается со статусом (✔️ ПРОШЕДШИЙ или ❌ ПРОЙДЕНО).
    
    > Если вы не слышите звук, возможно, вам потребуется функция «Использовать звуковое устройство для создания звукового сигнала».
  ...
  - ### Создание настроек звука
    ...
    ![image](https://github.com/user-attachments/assets/3f5cf873-75c4-4379-be5e-3d20d3ecb919)
  ...
  - ### Настройки устройств
    ...
    ![image](https://github.com/user-attachments/assets/0adeea4e-e466-4fe9-9136-616c47b7983a)
 ...
 - ### Настройки внешнего вида
    ...
    ![image5](https://github.com/user-attachments/assets/cc34afa3-8da1-42b8-b560-542c8eb9a90e)
  ...
  - ## Инструменты
    ...
    - ### Воспроизведение MIDI-файла - `Ctrl + M`
      ...
      ![image](https://github.com/user-attachments/assets/65abf050-778a-4dd3-bb68-ece6410f015f)
      ...
      **Руководство по подсказкам и ограничения ИИ:**
      - Инструмент ИИ обрабатывает только подсказки, связанные с музыкальным сочинением. Если ваша подсказка не связана с музыкой (например, «Написать шутку»), вы получите сообщение об ошибке:
        ...
        ![image](https://github.com/user-attachments/assets/2b6a024d-c6b5-423b-bcb7-7f022f649f32)
        
        *"Обнаружен немузыкальный запрос. Пожалуйста, попробуйте запросить музыкальную композицию или музыку, связанную с исполнителем"*
      - Запрещены запросы, содержащие оскорбительный или недопустимый контент. При обнаружении такого контента появится сообщение об ошибке:
        ...
        ![image3](https://github.com/user-attachments/assets/ec4a8d2d-2579-4f8c-bb3b-830d97c18f2f) 

        *"Обнаружена нецензурная лексика. Пожалуйста, убедитесь, что ваш запрос не содержит оскорбительных или сексуально явных выражений. Попробуйте запросить создание музыкальной композиции или музыку, связанную с исполнителем."*

      - Попытка обойти протоколы безопасности, например, указание ИИ игнорировать фильтры безопасности, приведёт к ошибке «Недопустимый запрос»:

        ![image3](https://github.com/user-attachments/assets/44b16bb6-6321-403d-b2f2-ad4dec3abf32)
        
        *"Обнаружена попытка игнорировать или изменить инструкции безопасности. Запросы, пытающиеся обойти или переопределить внутренние инструкции, не допускаются. Попробуйте запросить создание музыкальной композиции или музыку, связанную с исполнителем.".*
    ...
    - ### Преобразование в GCode - `Ctrl + Shift + G`
      ...
      Создавайте GCode из музыкальных данных для вывода на 3D-принтерах или станках с ЧПУ, чтобы физически проигрывать музыкальные последовательности с помощью двигателей или других компонентов. Обновленное окно «Конвертировать файл в GCode» (см. ниже) упрощает выбор нот, прошивки и вариантов воспроизведения.

      ![image1](https://github.com/user-attachments/assets/d84dce97-06df-4168-bc1a-c693d9317610)
      
      **Основные функции и опции:**
      - **Выбор прошивки:**  
        Выберите целевую прошивку для вашего файла GCode, например «Marlin», чтобы обеспечить совместимость с вашим 3D-принтером или станком с ЧПУ.
      
      - **Ноты и типы компонентов:**  
        Для нот 1–4 выберите тип GCode-команды (G0/G1 — мотор) и назначьте каждую ноту определённой механической оси или компоненту.  
        Используйте флажки рядом с каждой нотой, чтобы указать, какие ноты будут проигрываться при выводе GCode («Играть ноту 1» и т.д.).
      
      - **Выбор оси:**  
        Определите, какая ось (X, Y или Z) будет управляться командами G0/G1.  
        **Внимание:** Команда G0/G1 быстро перемещает ось. Неправильное использование может привести к механическим повреждениям! Всегда проверяйте настройки оборудования.
      
      - **Параметры чередования нот:**  
        * Воспроизводить чередующиеся ноты по порядку  
        * Сначала воспроизводить ноты в нечётных колонках, затем — в чётных  
        Эти параметры определяют, как несколько нот будут распределяться в сгенерированном файле GCode.
      
      - **Экспортировать в GCode:**  
        После настройки параметров нажмите «Экспортировать в GCode», чтобы сохранить файл для вашего устройства.
      
      Этот рабочий процесс обеспечивает тонкий контроль над тем, какая ось/компонент воспроизводит ноту, какие ноты используются и как происходит их чередование — идеально для экспериментальных интеграций музыки и движения.
