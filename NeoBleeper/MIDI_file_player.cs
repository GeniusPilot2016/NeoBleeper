...
namespace NeoBleeper
{
    public partial class MIDI_file_player : Form
    {
        ...
        private async Task ProcessCurrentFrame()
        {
            ...
            {
                // Bu zaman damgasındaki tüm MIDI olaylarını al
                var eventsAtThisTime = _midiFile.Events.SelectMany(track => track)
                                                  .Where(ev => ev.AbsoluteTime == currentTime)
                                                  .ToList();
                ...
                foreach (var noteOff in noteOffEvents)
                {
                    ...
                    {
                        MIDIIOUtils.SendNoteOff(noteOff.NoteNumber, noteOff.Channel == 10 ? 9 : noteOff.Channel);
                    }
                }
                ...
                foreach (var noteOn in noteOnEvents)
                {
                    ...
                    if (_enabledChannels.Contains(noteOn.Channel))
                    {
                        ...
                        MIDIIOUtils.SendNoteOn(noteOn.NoteNumber, instrument, noteOn.Channel == 10 ? 9 : noteOn.Channel);
                    }
                }
            }
