![neobleeper_banner](https://github.com/user-attachments/assets/32b32e83-83b8-4da0-a6b7-0b0f87fc74fa)

# 🖥️🎶🎹🤖 NeoBleeper: Почти забытое оборудование встречает современные технологии
...
NeoBleeper — первое в мире программное обеспечение для создания мелодий с поддержкой искусственного интеллекта, использующее системный динамик (он же PC Speaker) на материнской плате. Оно создано по мотивам программы Bleeper Music Maker, которую [Robbi-985 (он же SomethingUnreal)](https://youtube.com/@SomethingUnreal) разработал в конце 2007 — начале 2008 года, но от которой отказались в 2011 году из-за того, что в новых версиях Windows звуковые сигналы направлялись на звуковое устройство по умолчанию, а не на системный динамик на материнской плате. NeoBleeper обеспечивает более стабильное воспроизведение музыки, решая проблему, связанную с направлением звуковых сигналов на звуковое устройство по умолчанию, а не на системный динамик на материнской плате в новых версиях Windows, обращаясь к физическому порту системного динамика 🖥️🎶*
...
[![Смотреть видео](https://img.youtube.com/vi/izg5NYQzuuM/maxresdefault.jpg)](https://youtube.com/shorts/izg5NYQzuuM?si=pmTgzoNnz_i_96gM)
...
- ### 🖥️ Удобный, знакомый, более эргономичный и более естественный интерфейс
...
- ### ⏱️ Настройки музыкального размера и BPM
...
- ### 🚀 Больше, чем просто ремейк
  ...
  - #### ⌨️ Вы также можете использовать обычную клавиатуру в качестве фортепиано.
...
Вы можете легко запустить NeoBleeper без компиляции, используя готовый исполняемый файл, доступный в разделе «Релизы».

### ⚙️ Инструкция по выполнению

1. **Перейдите на страницу релизов**
- Посетите страницу [Releases](https://github.com/GeniusPilot2016/NeoBleeper/releases) на GitHub.

2. **Скачайте ZIP-файл**
- Найдите последний релиз, затем скачайте ZIP-файл (например, `neobleeper-1.0.0-beta-x64.zip` или аналогичный, в зависимости от вашей системы).

3. **Распакуйте ZIP-файл**
- Щёлкните правой кнопкой мыши по загруженному ZIP-файлу и выберите «Извлечь всё...» (в Windows) или воспользуйтесь предпочитаемым вами ZIP-архиватором.

4. **Запустите исполняемый файл**
- Откройте распакованную папку.
- Перейдите в папку `NeoBleeper`
- Найдите и дважды щёлкните по файлу `NeoBleeper.exe`, чтобы запустить программу.

5. **(Необязательно) Разрешите запросы безопасности**
- Если Windows определит, что программа загружается из Интернета, вам может потребоваться принять запрос безопасности или нажать «Подробнее», а затем «Запустить в любом случае».

**Установка не требуется.**
Если у вас возникнут проблемы с запуском NeoBleeper, убедитесь, что ваша система соответствует перечисленным выше требованиям.
...
## ⭐️ Поддержите NeoBleeper!

Если вам нравится этот проект, пожалуйста, [поставьте звездочку ⭐️ на GitHub](https://github.com/GeniusPilot2016/NeoBleeper), чтобы помочь другим найти его!

## 🌟 Знаковое событие, вызывающее смешанные чувства

Пятая звезда в этом проекте была добавлена ​​в очень личный и трогательный момент: на рассвете, узнав о смерти моего отца, который находился за границей, [Handsome Gato Inc.](https://github.com/handsomegato) (компания, специализирующаяся на разработке WordPress и цифровом маркетинге) присвоила мне звезду, и вскоре после этого последовало около 20 новых загрузок.

Это напоминание о том, что открытый исходный код — это не только код и функции, но и личные истории и переживания, стоящие за каждым коммитом и каждой звездой. Спасибо, что были частью этого пути.
